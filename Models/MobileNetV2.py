...
class MobileNetV2(nn.Module):
    ...
    def forward(self, x, feature_out=False):
        out = F.relu(self.bn1(self.conv1(x)))  # out: n x 32 x 32 x 32
        out = self.layers(out)  # out: n x 320 x 8 x 8

        out = F.relu(self.bn2(self.conv2(out)))  # out: n x 1280 x 8 x 8
...
class EncoderMobileNetV2(nn.Module):
    ...
    def __init__(self, num_classes=10, custom_cfg=None):
        ...
        self.scale_transition_layers = [5, 12]
        self.conv2 = nn.Conv2d(320, 1280, kernel_size=1, stride=1, padding=0, bias=False)
        self.bn2 = nn.BatchNorm2d(1280)
        self.linear = nn.Linear(1280, num_classes)
    ...
    def load_checkpoint(self, checkpoint_dir):
        logging.warning('*' * 30)
        logging.warning('Encoder MobileNetV2:')
        ...
        if 'module' in list(checkpoint['model'].keys())[0]:
            ...
            if 'resnet' in list(checkpoint['model'].keys())[0]:
                ...
                for k, v in checkpoint['model'].items():
                    ...
                    new_dict[name] = v
            elif 'model' in list(checkpoint['model'].keys())[0]:
                new_dict = OrderedDict()
                for k, v in checkpoint['model'].items():
