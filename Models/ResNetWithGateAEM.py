...
class RealTimeSaliencyRBF(nn.Module):
    ...
    def calculate_rbf(self, saliency_params):
        ...
        xy = (14. * torch.tanh(params[:, :2] / 14) + 16.).to(self.args.device)  # we use tanh relative to the center of image and add 16 to make
        # coordinates relative to top-left. (the final values will be in the center 28*28 frame of the image)
        sigma = (torch.logaddexp(torch.zeros_like(params[:, 2]), params[:, 2]) + 1e-8).to(self.args.device)  # sigma = log(1 + exp(m_x))
        maps = []
    ...
    def coordinate_arrays(self):
        y_coordinates = ((torch.arange(32.)).repeat((32, 1))).to(self.args.device)
        x_coordinates = (torch.transpose((torch.arange(32.)).repeat((32, 1)), 1, 0)).to(self.args.device)
