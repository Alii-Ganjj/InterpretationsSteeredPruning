...
class ResNet(nn.Module):
    ...
    def zero_out_grad(self):
        for m in self.modules():
            if isinstance(m, nn.BatchNorm2d):
                ...
                m.weight.grad[:] = 0
                m.bias.grad[:] = 0
    def zero_out_gate(self):
        for m in self.modules():
            if isinstance(m, soft_gate):
                if m.weights.grad is not None:
                    m.weights.grad[:] = 0
    ...
    def adjust_gate_parameter(self):
        for m in self.modules():
            if isinstance(m, soft_gate):
                m.weights.data[m.weights.data < 0.01] = torch.zeros(m.weights.data.size()).uniform_(0.01, 0.1).cuda()[m.weights.data < 0.01]

                m.weights.data[m.weights.data>1] = 1
                m.weights.data[m.weights.data<0] = 0

                if m.weights.data.max()<0.5:
                    m.weights.data[m.weights.data==m.weights.data.max()]=1


    def get_gate_parameters(self, discrete=False, random=False):
        all_parameters = []
        for m in self.modules():
            if isinstance(m, soft_gate):
                if not discrete:
                    all_parameters.append(m.weights)
                else:
                    c_param = m.weights.cpu().data
                    if random:
                        c_param=custom_STE.apply(c_param, True)
                    else:
                        c_param[c_param>=0.5]=1
                        c_param[c_param<0.5]=0
                    all_parameters.append(c_param.long())
...
def cus_resnet34(pretrained=False, progress=True, **kwargs):
...
def cus_resnet50(pretrained=False, progress=True, **kwargs):
...
def cus_resnet101(pretrained=False, progress=True, **kwargs):
    r"""ResNet-50 model from
    `"Deep Residual Learning for Image Recognition" <https://arxiv.org/pdf/1512.03385.pdf>'_
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
