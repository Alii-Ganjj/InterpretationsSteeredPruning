...
from Models.ResNetWithGate import RealTimeSaliencyRBF
...
for batch_idx, (inputs, ori_inputs, targets) in enumerate(tqdm_loader):
    ...
    with torch.no_grad():
        _, mask = model(inputs, targets)

    all_masks.append(mask.cpu().squeeze())
...
torch.save(mask_dataset, directory + 'val_data_gau.pth.tar')
